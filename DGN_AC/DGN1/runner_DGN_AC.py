...
from DGN1.model import Actor,Critic,AttEncoder
    ...
    def run(self):
        ...
        while episode < self.num_episode:
            ...
            for epoch in range(self.train_epoch):
                ...
                Action = torch.Tensor(Action).unsqueeze(dim=2).to(self.device)
                # Values = torch.Tensor(Values).to(self.device)
                Reward = torch.Tensor(Reward).unsqueeze(2).to(self.device)
                Next_Obs = torch.Tensor(Next_Obs).to(self.device)
                Mat = torch.Tensor(Mat).to(self.device)
                Next_Mat = torch.Tensor(Next_Mat).to(self.device)
                Done = torch.FloatTensor(Done).unsqueeze(2).to(self.device)

                Obs_hybrid,Obs_attention=self.att_encoder(Obs,Mat)
                Obs_hybrid=Obs_hybrid.detach()
                next_Obs_hybrid,next_Obs_attention=self.att_encoder(Next_Obs,Next_Mat)
                next_Obs_hybrid=next_Obs_hybrid.detach()
                ...
                policy_loss=self.critic(Obs_hybrid,self.actor(Obs_hybrid).unsqueeze_(dim=2))
                policy_loss = -policy_loss.mean()

                next_Action=self.actor_tar(next_Obs_hybrid)
                next_Action.unsqueeze_(dim=2)
