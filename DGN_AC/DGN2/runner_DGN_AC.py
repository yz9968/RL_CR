...
from DGN2.model import Actor,Critic,AttEncoder
from DGN2.buffer import ReplayBuffer
...
class Runner_DGN_AC:
    def __init__(self, args, env):
        ...
        self.actor_tar=Actor(self.hidden_dim,self.n_action,self.hidden_dim).to(self.device)
        self.critic_tar=Critic(self.hidden_dim + 1,self.hidden_dim).to(self.device)
        self.att_encoder=AttEncoder(self.agent_num,self.n_obs,self.hidden_dim,self.n_action).to(self.device)
        
        self.save_path = self.args.save_dir + '/' + self.args.scenario_name +'{1}/{0}_agent_att/'.format(self.agent_num,args.attach_info)
        ...
        while episode < self.num_episode:
            ...
            while step < self.max_step:
                if not self.env.simulation_done:
                    ...
                    obs_hybrid = obs_hybrid.view((self.agent_num,self.hidden_dim)).detach()
                    actions=self.actor(obs_hybrid)
                    actions_list=list(torch.squeeze(actions).cpu().numpy())

                    for i, agent in enumerate(self.agents):
                        if np.random.rand() < self.epsilon:
                            actions_list[i] = np.random.randint(self.n_action)
                    next_obs, next_adj, reward, done_signals, info = self.env.step(actions_list)
                    
                    self.buffer.add(obs, actions_list, reward, next_obs, adj, next_adj,[info['simulation_done']]*self.agent_num)
                    obs = next_obs
                    adj = next_adj
            ...
            for epoch in range(self.train_epoch):
                Obs, Action,Reward, Next_Obs, Mat, Next_Mat, Done = self.buffer.getBatch(self.batch_size)
                ...
                Obs_hybrid,Obs_attention=self.att_encoder(Obs,Mat)
                Obs_hybrid = Obs_hybrid.view((self.batch_size*self.agent_num,self.hidden_dim)).detach()
                next_Obs_hybrid,next_Obs_attention=self.att_encoder(Next_Obs,Next_Mat)
                next_Obs_hybrid = next_Obs_hybrid.view((self.batch_size*self.agent_num,self.hidden_dim)).detach()
                # Obs_attention=F.log_softmax(Obs_attention,dim=2)
                # critic_loss_kl = F.kl_div(critic_attention, target_critic_attention.detach(), reduction='batchmean')

                policy_loss=self.critic(Obs_hybrid,self.actor(Obs_hybrid))
                policy_loss = -policy_loss.mean()

                next_Action=self.actor_tar(next_Obs_hybrid)
                target_Values=self.critic_tar(next_Obs_hybrid,next_Action.detach())
                expected_Values= Reward + (1-Done)*self.gamma * target_Values
                Values=self.critic(Obs_hybrid,Action)
