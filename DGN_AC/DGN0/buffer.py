...
class ReplayBuffer(object):
	...
	def __init__(self, buffer_size, obs_space, n_action, n_agent):
		...
		self.value = np.zeros((self.buffer_size,self.n_agent), dtype='float32')
		self.rewards = np.zeros((self.buffer_size,n_agent),dtype='float32')
	...
	def getBatch(self, batch_size):
		...
		return self.obs[index], self.actions[index], self.value[index],self.rewards[index], self.next_obs[index], self.matrix[index], self.next_matrix[index] , self.dones[index]

	def add(self, obs, action,value, reward, next_obs, matrix, next_matrix, done):
